    ...
    if (file1.is_open()) {
        ...
        while (std::getline(file1, line)) {
            ...
            if (tokens.size() >= 5)
            {
                ...
                std::transform(node1_name.begin(), node1_name.end(), node1_name.begin(), ::toupper);
                std::string node2_name = tokens[3];
                std::transform(node2_name.begin(), node2_name.end(), node2_name.begin(), ::toupper);
                ...
                // lambda函数：检查首字母之后的部分是否全为纯数字
                auto is_pure_node = [](const std::string &s)
                {
                    if (s.length() <= 1)
                        return false;
                      ...
                      std::vector<std::vector<std::string>> &pairs_col_info)
{
    ...
    if (file2.is_open()) {
        ...
        while (std::getline(file2, line)) {
            auto tokens = parseCSVLine(line);
            if (tokens.size() >= 7) {
                std::string pair_col2 = tokens[1];
                std::string pair_col3 = tokens[2];
                std::vector vec = {pair_col2, pair_col3};
                pairs_col_info.push_back(vec);
                        ...
                        std::vector<std::vector<std::string>> pairs_col_info)
{
    ...
    for (const auto &pair2edges : pairs_to_ret_edges)
    {
        ...
        const auto &pair_col_info = pairs_col_info[pair_id];
        std::string col2 = pair_col_info[0];
        std::string col3 = pair_col_info[1];
...
int main(int argc, char* argv[]) {
    ...
    if (read_csv_to_pairs(file_path2, node_name_to_id, pairs, ori_pairs, pairs_to_weight, pairs_to_center_pairs, center_pairs,
                          ...
                          entry_edges, cpid_to_entry_edges, pairs_col_info) == -1)
    {
        return 1;
    }
    ...
    print_pairs_to_path(ori_pairs, id_to_ret_edges_prime, id_to_node_name, suffix, pairs_col_info);
