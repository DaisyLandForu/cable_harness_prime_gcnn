...
SCIP_DECL_BRANCHEXECLP(RlGcnnBranchrule::scip_execlp) {
    ...
    *result = SCIP_DIDNOTRUN;
    ...
    try {
        return_code = extractGraphObservation(scip, observation);
...
void RlGcnnBranchrule::writeLogRow(
    ...
    log_stream_
        ...
        << csvEscape(selected == nullptr ? "" : observation.candidate_names[selected_index]) << ',';
    if (selected == nullptr) {
        log_stream_ << ",,,";
    } else {
        log_stream_
...
SCIP_RETCODE includeRlGcnnBranchrule(
    ...
    SCIP_CALL(SCIPincludeObjBranchrule(
        scip,
        new RlGcnnBranchrule(scip, options, stats),
        TRUE));
    return SCIP_OKAY;
}
