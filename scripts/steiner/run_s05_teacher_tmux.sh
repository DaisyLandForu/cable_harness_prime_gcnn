...
readonly RUN_DIR="${REPO_ROOT}/results/steiner/raw/s05/s05-teacher-il-pilot-v3"
readonly LOG_PATH="${RUN_DIR}/tmux.log"
mkdir -p -- "$RUN_DIR"
readonly RUN_COMMAND="set -o pipefail; cd '${REPO_ROOT}' && scripts/steiner/run_with_scip804.sh --python scripts/steiner/collect_s05_teacher.py --config configs/steiner/experiments/s05_teacher_il_pilot_v3.yml --max-workers '${workers}' 2>&1 | tee -a '${LOG_PATH}'; code=\$?; printf 'S05_TEACHER_EXIT_CODE=%s\n' \"\$code\"; exec \${SHELL:-/bin/bash}"
